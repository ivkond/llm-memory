# RM-0003: Application-level write coordination
...
Add an application-level coordination layer on top of Git, such as local file locking or a single-writer queue, so concurrent write operations are serialized or retried safely.
