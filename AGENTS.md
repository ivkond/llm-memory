...
- Respond to the user in Russian unless they explicitly request another language.
...
- Do not touch protected files (`.env*`, `ci/**`, Docker/K8s/Terraform) without an explicit request.
...
This project is configured to use **bd (beads)** for task tracking when a beads database is available.
...
- If `bd` reports that no beads database exists, mention that in the handoff and continue with the user's requested work.
- Use `bd remember` for persistent project knowledge when beads is available; do not create ad-hoc memory files.
...
4. Push to remote:
   ```bash
   git pull --rebase
   bd dolt push  # if beads is available
   git push
...
Use focused checks while iterating, then broader checks before completion when code changes.

## Project Overview

**LLM Wiki** is a personal knowledge base for AI agents implementing Andrej Karpathy's LLM Wiki pattern.
...
- ESM only; include explicit `.js` extensions in relative imports from TypeScript source.
- Use `import type` for type-only imports under `verbatimModuleSyntax`.
- Named exports only; no default exports in source.
- Domain entities expose readonly data and enforce invariants through constructors/factories.
- Service methods accept request objects rather than long positional argument lists.
- All I/O APIs are async.
- No `console.*` in production code unless the transport layer intentionally writes user-facing output or stderr logs.
